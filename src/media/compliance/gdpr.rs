...
impl MediaGDPRCompliance {
    ...
    pub async fn process_media_access_request(
        ...
        let categories: Vec<PersonalDataCategory> = personal_data_items.iter().map(|item| item.data_category.clone()).collect();
        let retention_periods = self.get_retention_periods_for_categories(&categories).await?;
