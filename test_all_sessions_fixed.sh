...
LOG_DIR="/tmp/nano-messenger-tests"
mkdir -p "$LOG_DIR"
...
run_test() {
    ...
    local log_file="$LOG_DIR/test_$(date +%s)_$$.log"
    ...
    if eval "$test_command" > "$log_file" 2>&1; then
        ...
        if grep -q "COMPLETE\|SUCCESS\|✅" "$log_file" 2>/dev/null; then
...
test_sessions() {
    ...
    if [ -f "examples/session12_validation.rs" ]; then
